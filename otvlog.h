...
#define O_ASSERT(cond) STMT(if (!(cond)) {                                 \
    ...
    g_print( "ASSERT fail - file:%s,func:%s,ln:%d cond:%s\n",              \
    __FILE__,__func__,__LINE__,#cond );                                    \
...
#define O_WARN(cond) STMT(if (!(cond)) {                                              \
    ...
    __FILE__,__func__,__LINE__,#cond );                                               \
    g_print( "WARNING condition not met - file:%s,func:%s,ln:%d cond:%s\n",           \
    __FILE__,__func__,__LINE__,#cond ); })
...
/* TODO: Why are we doing this crap in the file. Just fix the damn makefiles */
...
#else
    /* Null out the definitions if SYSLOG output is disabled */
    #define O_LOG_SYSLOG(lvl,msg,args...)
    #define O_LOG_SYSLOG_DBG(lvlname,lvl,msg,args...)
    #define O_LOG_SYSLOG_DBG_SIMPLE(lvlname,lvl,msg,args...)
    #define O_LOG_SYSLOG_EX(lvlname,lvl,msg,args...)
     ...
     */
        ...
        #define O_LOG_PRINT_DBG(lvlname, msg,args...) \
                g_print(lvlname ": ["COMP_NAME"] %s():line %i - " msg,__func__,__LINE__, ## args);
    #else
        #define O_LOG_PRINT_DBG(lvlname, msg,args...) \
                g_print(lvlname ": ["COMP_NAME"] " __FILE__  ",line %i - " msg,__LINE__, ## args);
    #endif

    #define O_LOG_PRINT_DBG_SIMPLE(lvlname, msg,args...) \
        g_print(lvlname ": ["COMP_NAME"] " msg, ## args);
    ...
    #define O_LOG_PRINT_EX(lvlname, msg,args...) \
        g_print(lvlname ": ["COMP_NAME"] " __FILE__ ",%s,line %i - " msg,__func__, __LINE__, ## args);
#else
    /* Null out the definitions if STDOUT output is disabled */
    #define O_LOG_PRINT_DBG(lvlname, msg,args...)
    #define O_LOG_PRINT_DBG_SIMPLE(lvlname, msg,args...)
    #define O_LOG_PRINT_EX(lvlname, msg,args...)
#endif
...
#else
    #define O_LOG_ERROR(msg,args...)
    #define O_LOG_ERROR_SIMPLE(msg,args...)
#endif
/**
...
#else
    #define O_LOG_WARNING(msg,args...)
    #define O_LOG_WARNING_SIMPLE(msg,args...)
#endif
/**
...
#else
    #define O_LOG_MAINSTATUS(msg,args...)
    #define O_LOG_MAINSTATUS_SIMPLE(msg,args...)
#endif
/**
...
#else
    #define O_LOG_DEBUG(msg,args...)
    #define O_LOG_DEBUG_SIMPLE(msg,args...)
#endif
/**
...
#else
    #define O_LOG_TRACE(msg,args...)
    #define O_LOG_TRACE_SIMPLE(msg,args...)
#endif
...
#define REQUIRE(pre) STMT(if (!(pre)) {                                     \
    ...
    g_print( "Precondition fail - file:%s,func:%s,ln:%d cond:%s\n",         \
        __FILE__,__func__,__LINE__,#pre );                                  \
    fflush( stdout );assert(0); })
...
#define ENSURE(post) STMT(if (!(post)) {                                    \
    ...
    g_print( "Postcondition fail - file:%s,func:%s,ln:%d cond:%s\n",        \
        __FILE__,__func__,__LINE__,#post );                                 \
    fflush( stdout );assert(0); })
